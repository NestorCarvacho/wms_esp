...
async def listar_notificaciones(
    ...
    _: dict = Depends(requiere_permiso("notificaciones.leer")),
...
async def contar_no_leidas(
    ...
    try:
        resultado = await handlers.contar_no_leidas.handle(usuario_autenticado["id"])
...
async def marcar_leida(
    ...
    try:
        resultado = await handlers.marcar_leida.handle(id, usuario_autenticado["id"])
...
async def marcar_todas_leidas(
    ...
    try:
        resultado = await handlers.marcar_todas_leidas.handle(usuario_autenticado["id"])
        return RespuestaAPIDTO(exito=True, datos=resultado, mensaje=resultado["mensaje"]).model_dump()
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
