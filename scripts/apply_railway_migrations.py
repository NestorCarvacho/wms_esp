...
def run_diagnostics(cursor) -> None:
    checks = [
        ...
        ("Tabla region", "SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='region'"),
        ("Columna empresa.razon_social", "SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='empresa' AND COLUMN_NAME='razon_social'"),
        ("Tabla notificacion (debe ser 0)", "SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='notificacion'"),
        ("Tabla password_reset_token (debe ser 0)", "SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='password_reset_token'"),
        ("Índice uk_producto_sku_empresa", "SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='producto' AND INDEX_NAME='uk_producto_sku_empresa'"),
